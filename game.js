...
let player={x:W*.5,y:H*.72,a:-Math.PI/2,hp:140,max:140,speed:155,r:25};
let enemies=[],bullets=[],particles=[];
function resetWave(){player.x=W*.5;player.y=H*.72;let n=4+Math.min(5,wave);enemies=[];for(let i=0;i<n;i++) enemies.push({x:70+Math.random()*(W-140),y:145+Math.random()*Math.max(100,H*.42),a:Math.PI/2,hp:wave%5===0&&i===0?220:70,max:wave%5===0&&i===0?220:70,r:24,boss:wave%5===0&&i===0,cd:400+Math.random()*700,type:1+i%3});updateHUD()}
...
function drawSprite(im,o,size){
 ctx.save();ctx.translate(o.x,o.y);ctx.rotate(o.a+Math.PI/2);
 ctx.shadowColor='#000c';ctx.shadowBlur=14;ctx.shadowOffsetY=9;
 let ratio=im.width/im.height,w=size*ratio,h=size;ctx.drawImage(im,-w/2,-h/2,w,h);ctx.restore()
}
function draw(){
 ...
 enemies.forEach(e=>{let im=e.boss?imgs.boss:imgs['e'+e.type];drawSprite(im,e,e.boss?100:82);ctx.fillStyle='#351013';ctx.fillRect(e.x-27,e.y-46,54,7);ctx.fillStyle=e.boss?'#ff9a2d':'#ff4047';ctx.fillRect(e.x-27,e.y-46,54*Math.max(0,e.hp/e.max),7)});
 drawSprite(imgs.player,player,96);ctx.strokeStyle='#39fff0aa';ctx.lineWidth=2;ctx.shadowColor='#35fff0';ctx.shadowBlur=18;ctx.beginPath();ctx.arc(player.x,player.y,37,0,7);ctx.stroke();ctx.shadowBlur=0;
